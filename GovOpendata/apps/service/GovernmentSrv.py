...
class GovernmentSrv(BaseSrv):
    orm = Government

    @classmethod
    def statistics(cls) -> object:
        all_government = cls.orm.all(_to_dict=False)
        dataset_num = 0
        file_num = 0
        file_size = 0
        gov_dict = {}
    ...
    def update(cls, province: str, region: str, dir_path: str, file_num: int,
        ...
        try:
            cls.orm.query.filter_by(dir_path=dir_path).update(
                {
                    "province": province,
                    "region": region,
                    "dir_path": dir_path,
    ...
    def find_by_dir_path(cls, dir_path: str) -> bool:
        return cls.orm.query.filter_by(dir_path=dir_path).first()

    @classmethod
    def get_id(cls, dir_path: str) -> int:
        obj = cls.orm.query.filter_by(dir_path=dir_path).first()
